...
tab1, tab2 = st.tabs(["Transcribe and Translate", "Translation Test"])

# TAB 1: Transcribe and Translate
with tab1:
    with st.container():
    ...
    # Execution button for Tab 1
    if st.button("Start the Analysis Process", type="primary", use_container_width=True, key="btn_pipeline"):
...
# TAB 2: Translation Test
with tab2:
    st.subheader("Isolated Translation Testing")
    st.markdown("Use this section to test the LLM translation models directly without running the ASR engine.")
    
    text_input = st.text_area(
        "Input Text to Translate:", 
        height=150, 
        placeholder="Enter your raw Dioula, Mooré, French, or English text here..."
    )
    ...
    with col_b:
        tr_source = st.selectbox(
            "Source Language:", 
            options=["auto", "french", "english", "moore", "dioula"],
            key="playground_source_select"
        )
    with col_c:
        tr_target = st.selectbox(
            "Target Translation:", 
            options=["English", "French"],
            key="playground_target_select"
        )

    if st.button("Translate Text", type="primary", use_container_width=True, key="btn_translate"):
        if text_input.strip():
            with st.spinner(f"Translating using {tr_model}..."):
                
                result = modules["translator"].translate_text(
                    text=text_input,
                    source_lang=tr_source,
                    target_lang=tr_target,
                    model_choice=tr_model
                )
                
                st.success("Translation Completed!")
                st.markdown("**Result:**")
                st.info(result.translated_text)
        else:
