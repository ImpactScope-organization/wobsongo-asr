...
def compute_meaning_preserved_chrf(hyps_raw, dataset, mt_model_path, target_column=MT_REFERENCE_COLUMN):
    ...
    try:
        ...
        print("[WARN] sacrebleu / transformers not available, skipping chrF check.")
        return None

    print("\n[INFO] Loading NLLB model for meaning preservation check...")
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    
    tokenizer = AutoTokenizer.from_pretrained(mt_model_path, src_lang="dyu_Latn")
    model = AutoModelForSeq2SeqLM.from_pretrained(mt_model_path).to(device)

    fr_refs = dataset[target_column]
    translated_hyps = []
    
    print(f"[INFO] Translating {len(hyps_raw)} hypotheses to French...")
    for h in hyps_raw:
        if not h.strip():
            translated_hyps.append("")
            continue
        inputs = tokenizer(h, return_tensors="pt").to(device)
...
try:
    wandb.init(
        project="wobsongo-whisper-bambara-dioula-reeval_3",
        id="whisper-large-v2-bambara-dioula-v2-clean-reeval-3",
        resume="allow",
...
chrf_score = compute_meaning_preserved_chrf(best_pred_str_raw, dataset_dev, MT_MODEL_PATH)
...
if WANDB_ENABLED:
    try:
        ...
        if chrf_score is not None:
            wandb.summary["best_clean_dev_meaning_chrf"] = chrf_score
