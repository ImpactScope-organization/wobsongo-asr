...
app = modal.App("whisper-no-aug-dioula-finetune")
...
def train_whisper_remote():
    ...
    print("Initializing on Modal A100 GPU Server (NO AUGMENTATION)...")
    ...
    wandb.init(
        ...
        config={
            ...
            "method": "LoRA + Fixed Generation Rem Darurat (NO AUGMENTATION)",
            "dataset": "Mozilla-CV17 (272 rows) + UVCI-Koumankan Train (8065 rows)",
            "total_data_pool": 8337,
            "train_split_size": 6669,
            "test_split_size": 1668,
    ...
    training_args = Seq2SeqTrainingArguments(
        output_dir="/output/checkpoints_large_split8020_NO_AUG",
        ...
        logging_steps=50,
        report_to="wandb",
        run_name="whisper-large-v3-NO-AUG"
    )
    ...
    last_checkpoint = get_last_checkpoint("/output/checkpoints_large_split8020_NO_AUG")
    
    if last_checkpoint is not None:
        print(f"Resuming training from checkpoint: {last_checkpoint}")
        trainer.train(resume_from_checkpoint=last_checkpoint)
    else:
        print("Starting training from scratch for the new NO AUGMENTATION split...")
        trainer.train()

    print("Saving the final model to Volume...")
    final_output_path = "/output/whisper-large-v3-NO-AUG-final"
    trainer.save_model(final_output_path)
    processor.save_pretrained(final_output_path)
    model_volume.commit()
    
    print("Done! Model successfully trained WITHOUT augmentation.")
    wandb.finish()
